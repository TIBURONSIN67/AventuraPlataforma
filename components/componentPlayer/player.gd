...
@export var hit_box: HitBox = null  # Caja de colisión para ataques cuerpo a cuerpo
@export var health: Health = null  # Salud del jugador
...
@onready var particles_trail = $ParticlesTrail  # Efectos de partículas al moverse
@onready var walk_audio = $WalkAudio  # Sonido de caminar
@onready var character: Node3D = $Character  # Nodo de personaje
@onready var animation_tree: AnimationTree = $AnimationTree  # Árbol de animaciones
...
@onready var gui_player: Control = $GUI_player  # GUI del jugador
...
func _ready() -> void:
	...
	health.is_dead.connect(_is_dead)  # Conexión al evento de muerte del jugador
	health.updated_health.connect(_updated_heath)
...
func change_anim_move_state(new_state: String) -> void:
	...
	match target_index:
		...
		"Dead":
			...
			if animation_tree.get("parameters/Transition2/current_state") != "Dead":
				animation_tree.set(transition_path, target_index)
				print(target_index)
				current_animation_state = new_state
				return  # Evita transiciones inválidas
		
		"NotDead":
			transition_path = "parameters/Transition2/transition_request"
			if animation_tree.get("parameters/Transition2/current_state") != "NotDead":
				animation_tree.set(transition_path, target_index)
				current_animation_state = new_state
				return  # Evita transiciones inválidas
	
	# Si la animación de destino es diferente a la actual, aplicar la transición
	if target_index != "Jump" and target_index != "Dead":
...
func _idle_state(delta: float) -> void:
	if not is_on_floor() and not ray_cast.is_colliding():
		...
		change_state("Fall")
		return
	check_melee_attack_input(delta)
	input.x = 0
	input.z = 0
	jump_single = false
...
func check_player_move_input(delta):
	...
	elif Platform.is_android():  # Si está en Android
...
# Comprobación de entrada de ataque cuerpo a cuerpo
func check_melee_attack_input(delta):
